...
// MOCK — zone-level dwell/traffic tracking not implemented yet (Phase 3).
const MOCK_ZONES = [
...
export default function Statistics({ onNavigate }) {
    ...
    const [hourly, setHourly] = useState(Array.from({ length: 15 }, (_, i) => ({ hour: i + 8, avgPeople: 0 })));
    ...
    // Fetch hourly chart for "today" on first load
    useEffect(() => {
        fetchHourly(today);
    }, []);
    ...
    async function fetchHourly(date) {
        try {
            const res = await statsApi.getHourly(date);
            // Slice store-hours 08–22 to match the chart's visual range
            setHourly(res.data.filter((h) => h.hour >= 8 && h.hour <= 22));
        } catch (e) {
            // leave previous hourly data on screen
        }
    }
            ...
            if (mode === 'date') {
                ...
                const agg = list.reduce(
                    (acc, s) => {
                        ...
                        acc.totalDwellSeconds += s.totalDwellSeconds || 0;
                        acc.dwellSampleCount += s.dwellSampleCount || 0;
                        ...
                        acc.alertCounts.loitering += s.alertCounts?.loitering || 0;
                        acc.alertCounts.total += s.alertCounts?.total || 0;
                        return acc;
                    },
                    {
                        totalPeople: 0, maleCount: 0, femaleCount: 0, unknownGenderCount: 0,
                        totalDwellSeconds: 0, dwellSampleCount: 0,
                        alertCounts: { intrusion: 0, crowd: 0, abnormal: 0, loitering: 0, total: 0 },
                    }
                );
                agg.avgDwellSeconds = agg.dwellSampleCount > 0 ? agg.totalDwellSeconds / agg.dwellSampleCount : 0;
                setStats(agg);
                // Hourly chart not meaningful across a multi-day range — left as-is
            }
        } catch (e) {
    ...
    const hourlyMax = Math.max(1, ...hourly.map((h) => h.avgPeople));
    const alertBars = [
        ...
        { label: 'Loitering', value: ac.loitering ?? 0, color: 'var(--primary)' },
    ];
                ...
                <main className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {mode === 'date' ? (
                                <div className="flex items-center gap-2 rounded-md px-3 py-2 border" style={{ background: 'var(--input)', borderColor: 'var(--border)' }}>
                                    <input
                                ...
                                <div className="flex items-center gap-2">
                                    {[['From', from, setFrom], ['To', to, setTo]].map(([label, val, setter]) => (
                                        <div key={label} className="flex items-center gap-2 rounded-md px-3 py-2 border" style={{ background: 'var(--input)', borderColor: 'var(--border)' }}>
                            ...
                            <button onClick={handleQuery} disabled={loading} className="text-sm font-bold px-4 py-2 rounded-md border" style={{ borderColor: 'var(--border)', color: 'var(--foreground)' }}>
                                {loading ? 'Loading…' : 'View'}
                            </button>

                            <button onClick={handleReport} disabled={reporting} className="text-sm font-bold px-4 py-2 rounded-md flex items-center gap-2" style={{ background: 'var(--primary)', color: 'var(--primary-foreground)' }}>
                                <FileText size={14} />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="rounded-lg p-5 flex flex-col gap-3 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
                            <div className="text-xs tracking-widest font-bold uppercase" style={{ color: 'var(--muted-foreground)' }}>
                                People Count — Hourly {mode === 'range' && <span className="opacity-50">(single-day only)</span>}
                            </div>
                                ...
                                {hourly.map((h, i) => (
                                    <div key={h.hour} className="flex-1 flex flex-col items-center justify-end">
                                        <div
                                            ...
                                            style={{
                                                height: `${(h.avgPeople / hourlyMax) * 110}px`,
                                                background: i === hourly.length - 1 ? 'var(--primary)' : 'var(--secondary)',
                                            }}
                                        />
                                    </div>
                                ))}
                            ...
                            <div className="flex gap-1">
                                {hourly.map((h) => (
                                    <div key={h.hour} className="flex-1 text-center" style={{ fontSize: 9, color: 'var(--muted-foreground)' }}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="rounded-lg p-5 flex flex-col gap-3 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
                            ...
                            <div className="flex flex-col gap-2">
                                {[
                                    ...
                                    ['Loitering', ac.loitering ?? 0, 'var(--primary)'],
                        ...
                        <div className="rounded-lg p-5 flex flex-col gap-3 border" style={{ background: 'var(--card)', borderColor: 'var(--border)' }}>
                            ...
                            {/* MOCK — zone-level traffic tracking not implemented yet (Phase 3) */}
