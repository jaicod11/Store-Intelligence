...
const CHANNEL_DWELL = 'store:dwell';
...
async function connectRedis() {
    ...
    await subscriber.subscribe(CHANNEL_DWELL, async (message) => {
        try {
            const event = JSON.parse(message);
            await processDwellEvent(event);
        } catch (err) {
            console.error('Dwell event processing error:', err.message);
        }
    });

    console.log(`✅  Redis subscribed → ${CHANNEL_DETECTIONS}, ${CHANNEL_ALERTS}, ${CHANNEL_DWELL}`);
    return client;
}
